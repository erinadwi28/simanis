...
class Dashboard extends CI_Controller
{
        ...
        public function kirim_alasn_permohonan()
        {
                ...
                $email = $this->db->from('permohonan_ptsp');
                $email = $this->db->join('pemohon', 'permohonan_ptsp.id_pemohon = pemohon.id_pemohon', 'INNER');
                $email = $this->db->where('permohonan_ptsp.id_permohonan_ptsp', $detailhere)->result();
                // Konfigurasi email
                $config = [
                        'mailtype'  => 'html',
                        'charset'   => 'utf-8',
